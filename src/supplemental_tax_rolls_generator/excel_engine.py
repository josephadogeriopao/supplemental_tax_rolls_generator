...
def append_data_and_details(writer, df, data_sheet_name, details_sheet_name, source_file_path):
    ...
    🔥 UPDATED: Now returns a dictionary mapping (TaxYear, Column) -> Actual Python numeric sum value!
    """
    numeric_cols = [col for col in df.columns if pd.api.types.is_numeric_dtype(df[col])]
    exclude_cols = {'TAXYR', 'LUC', 'QUARTER'}
    cols_to_sum = [col for col in numeric_cols if col not in exclude_cols]
    ...
    # 🔥 This will now hold raw numeric sums instead of string cell references
    subtotal_values_by_year = {}
    ...
    for tax_year, group in df.groupby('TAXYR'):
        ...
        for col_idx, col_name in enumerate(df.columns, start=1):
            col_letter = get_column_letter(col_idx)
            if col_name in cols_to_sum:
                # 1. Leave the Excel formula in the sheet cells for users looking at the sheet
                formula = f"=SUM({col_letter}{start_data_row}:{col_letter}{end_data_row})"
                cell = worksheet.cell(row=current_row, column=col_idx, value=formula)
                cell.font = bold_font
                cell.number_format = num_format

                # 2. 🔥 FIX: Compute the genuine raw numeric total sum using pandas for Python to use
                raw_pandas_sum = group[col_name].fillna(0).sum()

                # Format to a clean integer if it is a whole number
                if float(raw_pandas_sum).is_integer():
                    raw_pandas_sum = int(raw_pandas_sum)
                else:
                    raw_pandas_sum = float(raw_pandas_sum)

                # Save the real aggregate number to our return dictionary
                subtotal_values_by_year[(int(tax_year), col_name)] = raw_pandas_sum
    ...
    # 🔥 Now returns numbers instead of strings!
    return subtotal_values_by_year
